...
ED.clean$dischargehour=hour(ED.clean$DISCHARGE_DATE_TIME)
...
# Mark's test code
plot_tmp = reshape2::melt(plot, id.vars = c("monthofrequest", "count"))

group.colors = c(mean_tot = "black", mean_dur1 = "red", mean_dur2 = "green")

ggplot(data = plot_tmp, aes(x = monthofrequest, y = value, size = count)) +
  geom_point() +
  scale_fill_manual(values = group.colors)

#PLOT: Does Duration vary by Day of the Week
pdf('plotweekdayboxplot.pdf')
...
#Plot:  Does duration vary by Inpatient Team? Yes
plot2 <-ED.clean.3 %>%
...
ggplot()+
  ...
  geom_point(data=plot2,aes(x=INP_TEAM_RENAMED, y=mean_dur1, color="green")) +
  geom_point(data=plot2,aes(x=INP_TEAM_RENAMED, y=mean_dur2,color="red")) +
       theme(axis.text.x = element_text(face="plain", color="#993333", size=10, angle=45, hjust=1),
             axis.text.y = element_text(face="plain", color="#993333", size=10, angle=45, hjust=1)) +
...
#multivariable continuous regression
...
ED.svm<- svm(tot.dur ~ TEAM_ED + INPATIENT_FLOOR + INPATIENT_SERVICE+ dayofweek + timeofday + monthofrequest + tot_ED_admits.day + tot_discharges + INP_TEAM_RENAMED, data=data.ED.svm, scale=TRUE, kernel="radial") #polynomial, radial did not find any outliers, did not fit the data well
ED.svm
