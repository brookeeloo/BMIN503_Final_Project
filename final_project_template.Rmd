...
#NOTES TO SELF:
# surgical ED visits are counted twice - once to periop and once to 4ST, need to rectify this
```  
***
...
for(i in 1:nrow(ED.clean.3)){
  a<-grepl('asthma|respi|bronchiolitis|wheezing|dyspnea', ED.clean.3$DIAGNOSIS[[i]], ignore.case = TRUE)
...
#Boxplot of average wait times:
summary(ED.clean.3$tot.dur)

ggplot(data=ED.clean.3, aes(y=tot.dur)) +
      geom_boxplot() +
...
#PLOT: Does Duration vary by Day of the Week
  ggplot(data=ED.clean.3, 
         aes(x=factor(dayofweek, levels=c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday")), y=tot.dur)) + 
    geom_boxplot() +
    scale_x_discrete("days of the week") +   #labels
...
ggplot(data=plot2,
       aes(x=INP_TEAM_RENAMED, y=mean)) + geom_point() +
       theme(axis.text.x = element_text(face="plain", color="#993333", 
                           size=10, angle=45),
             axis.text.y = element_text(face="plain", color="#993333", 
                           size=10, angle=45))
...
#plot 4: Does it vary by time of day that the admission bed request was placed?  yes
plot3<-ED.clean.3 %>%
  group_by(timeofday) %>%
  summarise(mean=mean(tot.dur), count=n())
ggplot(data=plot3,
       aes(x=timeofday, y=mean)) + geom_point() +
       theme(axis.text.x = element_text(face="plain", color="#993333", 
                           size=10, angle=45),
             axis.text.y = element_text(face="plain", color="#993333", 
                           size=10, angle=45))

#plot 5:  Does it vary by the number of admissions that day
plot4 <- ED.clean.3 %>%
  group_by(grouped.diagnoses) %>%
  summarise(mean=mean(tot.dur), count=n())
plot4<-plot4[plot4$count>100,]
ggplot(data=plot4,
       aes(x=grouped.diagnoses, y=mean)) + geom_point() 
       #theme(axis.text.x = element_text(face="plain", color="#993333", 
       #                    size=10, angle=45),
      #       axis.text.y = element_text(face="plain", color="#993333", 
       #                    size=10, angle=45))
