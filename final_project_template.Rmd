...
#upload and configure the data
#filename: MDrptEtc_20181027_FY17on
admissions.data <- read.csv(file="MDrptEtc_20181027_FY17on.csv", header=TRUE)
ED.data<-read.csv(file="MDrptWithBedReqandAdmitTeam_20181104_FY17on.csv", header=TRUE)

#checking header names
a=colnames(admissions.data)
b=colnames(ED.data)
names<-cbind(a,b)

#rename column headings
admissions.data.renamed <- rename(admissions.data,
                                  ...
                                  ADMIT_INPATIENT_DATE_TIME=CENSUS_ADMIT_TM,  #proxy time pt arrived on the floor
                                  DIAGNOSIS = ACCT_ADMIT_DX_NM, #diagnosis from the ED
                                  MRN = PAT_MRN_ID,  #Identifier
                                  ...
                                  INPATIENT_TEAM=CENSUS_ADMIT_SERVCIE,  #inpatient team who admitted the patient
                                   )
...
#crime_date = as.Date(ymd_hms(ED.data.renamed$time_bed_assigned))
#summary(crime_date)
