...
Objective: To determine if 1) we can identify variables that seem to significantly impact ED wait times, 2) determine if machine learning can provide a framework for finding variables that most significantly influence ED admission wait times and therefore direct QI efforts.
...
This project is interdisciplinary because it is using the concepts of descriptive analysis and machine learning, but adapted into a well established hospital quality improvement framework used throughout the world today for process improvement.  I
...
ED.clean.2b=inner_join(ED.clean.2, discharges.per.day, b="bed_request_date")
...
ggplot()+
  ...
  scale_colour_manual(name = 'color', values=c('black','green','red'),labels = c('total wait time','bed request to MD report','MD report to arrival to floor'))
...
ggplot()+
  ...
  geom_point(data=plot1,aes(x=INPATIENT_FLOOR, y=mean_dur1, color="green")) +
...
ggplot()+
  ...
  #geom_point(data=plot,aes(x=INP_TEAM_RENAMED, y=mean_dur1, color="green")) +
...
plot <- ED.clean.3 %>%
  ...
  summarise(mean_tot=mean(tot.dur), mean_dur1=mean(dur1.request.report), mean_dur2=mean(dur2.report.floor), count=n())
plot<-plot[plot$count>100,]
ggplot()+
  geom_point(data=plot,aes(x=reorder(grouped.diagnoses,-mean_tot),y=mean_tot, color="black",size=count))  +
  geom_point(data=plot,aes(x=grouped.diagnoses, y=mean_dur1, color="green",size=count)) +
...
plot <- ED.clean.3 %>%
  ...
  summarise(mean_tot=mean(tot.dur), mean_dur1=mean(dur1.request.report), mean_dur2=mean(dur2.report.floor), count=n())
ggplot()+
  geom_point( data=plot, aes(x = tot_discharges, y = mean_tot, size=count))+
  theme(axis.text.x = element_text(face="plain", color="#993333", size=6, angle=70, hjust=1),
        axis.text.y = element_text(face="plain", color="#993333",size=6, angle=45))+
...
summary((lm(tot.dur~ TEAM_ED + INPATIENT_FLOOR + INPATIENT_SERVICE + dayofweek + timeofday + monthofrequest + bed_request_date + tot_ED_admits.day + tot_discharges +INP_TEAM_RENAMED, data=ED.clean.3, family=binomial(logit))))
head(ED.clean.3)
...
We've looked at the enterprise level of data, now lets look at the Service level.
```{r}
genpeds.data.clean <- ED.clean.3 %>%
  filter(INPATIENT_SERVICE == "General Pediatrics") %>%
  return
columnnames=colnames(genpeds.data.clean)
...
ggplot()+
      geom_point(data=plot, aes(x=INPATIENT_FLOOR, y=mean_tot, size=count))+
       theme(axis.text.x = element_text(face="plain", color="#993333", size=10, angle=60, hjust=1),
             axis.text.y = element_text(face="plain", color="#993333", size=10, angle=45))+
...
plot<- fracture.data.clean %>%
  group_by(INPATIENT_FLOOR) %>%
  summarise(mean_tot=mean(tot.dur), mean_dur1=mean(dur1.request.report), mean_dur2=mean(dur2.report.floor), count=n())
plot<-plot[plot$count>5,]
#Asthma ED wait time by floor
ggplot()+
      geom_point(data=plot, aes(x=reorder(INPATIENT_FLOOR,-mean_tot), y=mean_tot, size=count))+
       theme(axis.text.x = element_text(face="plain", color="#993333", size=10, angle=60, hjust=1),
             axis.text.y = element_text(face="plain", color="#993333", size=10, angle=45))+
       xlab("Inpatient Floor")+ ylab("Avg Wait Time (min)")+ ggtitle("Wait Time by Inpatient Floor")
#       geom_bar(position="dodge") +
...
data.ED.svm<- ED.clean.3 %>%
  ...
  mutate(tot.dur = cut(tot.dur, breaks=c(0, 140, 1000), labels=c("control", "outlier")))
#data.ED.svm$poop<-as.character(data.ED.svm$poop)
...
ED.svm<- svm(tot.dur ~., data=data.ED.svm, scale=TRUE, kernel="radial") #polynomial, radial did not find any outliers, did not fit the data well
ED.svm
...
for(i in 1:K){
    ...
    obs.outputs[1:length(s[s==i]) + offset] <- data.ED.svm$status  #length of test data
    
    #GLM train/test
	glm <- glm(status~., data=train.hw, family=binomial(logit))  #1. prediction model 
  ...
  hw.rf <- randomForest(status~., data=train.hw, ntree=100)
  rf.pred.curr <- predict(hw.rf, newdata=test.hw, type="prob")
...
plot.roc(data.hw$status, glm.pred, ci=TRUE)  #compare apples to apples (true data, compared to predicted data that used 100% of variables)
plot.roc(obs.outputs, pred.outputs.glm, col="red", add=TRUE)

#random forest
roc(obs.outputs, pred.outputs.rf, ci=TRUE)  #
plot.roc(data.hw$status, rf.pred, add=TRUE)  #training ROC curve
