...
ggplot(data=plot5, aes(x=year, y=dur1.request.report, group=year)) +
      geom_boxplot() +
      theme(axis.text.y = element_text(face="plain", size=10, angle=45))

#Does duration vary by month? not really
plot6<- ED.clean.3 %>%
...
ggplot(data=plot1,
       aes(x=reorder(INPATIENT_FLOOR,-mean), y=mean)) + geom_point() +
...
#Inpatient TEAM average plot
par(mfrow=c(2,1))
ggplot(data=plot2,
       aes(x=reorder(INP_TEAM_RENAMED,-mean), y=mean)) + geom_point() +
...
ggplot(data=plot3,
       ...
       xlab("Inpatient Team") + ylab("Average Wait Time (min)") +ggtitle("Wait Time vs Hour of Bed Request")
...
ggplot(data=plot4,
       aes(x=reorder(grouped.diagnoses,-mean), y=mean)) + geom_point() +
